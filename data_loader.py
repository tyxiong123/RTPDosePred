...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cfig['CatStructures']:
            ...
            data_dict['data'] = torch.cat((comb_optptv,  comb_oar, In_dict['Body'], In_dict['img'], data_dict['beam_plate']), axis=0) # , In_dict['obj_2DGy'], In_dict['obj_2DWei']
...
if __name__ == '__main__':
    ...
    #     data = next(iter(train_loader))
    # time_end = time.time()
    # print('Time cost: ', time_end - time_start) # 284
    # for i, data in enumerate(train_loader):

    #    # pdb.set_trace()
    #     print (data['data'].shape, data['label'].shape, data['beam_plate'].shape, data['prompt'].shape, data['ori_img_size'])

    #     beam_plate = data['data'][0,0,:,:,:].numpy()
    #     print(beam_plate.shape)
    #     print(np.max(beam_plate), np.min(beam_plate))
    #     plt.imshow(beam_plate[48])
    #     plt.colorbar()
    #     plt.show()

    #     beam_plate = data['data'][0,3,:,:,:].numpy()
    #     print(beam_plate.shape)
    #     print(np.max(beam_plate), np.min(beam_plate))
    #     plt.imshow(beam_plate[48])
    #     plt.colorbar()
    ...
    #     beam_plate = data['data'][0,4,:,:,:].numpy()
    #     print(beam_plate.shape)
    #     print(np.max(beam_plate), np.min(beam_plate))
    #     plt.imshow(beam_plate[48])
    #     plt.colorbar()
    ...
    #     beam_plate = data['data'][1,0,:,:,:].numpy()
    #     print(beam_plate.shape)
    #     print(np.max(beam_plate), np.min(beam_plate))
    #     plt.imshow(beam_plate[48])
    #     plt.colorbar()
    ...
    #     beam_plate = data['data'][1,3,:,:,:].numpy()
    #     print(beam_plate.shape)
    #     print(np.max(beam_plate), np.min(beam_plate))
    #     plt.imshow(beam_plate[48])
    #     plt.colorbar()
    ...
    #     beam_plate = data['data'][1,4,:,:,:].numpy()
    #     print(beam_plate.shape)
    #     print(np.max(beam_plate), np.min(beam_plate))
    #     plt.imshow(beam_plate[48])
    #     plt.colorbar()
