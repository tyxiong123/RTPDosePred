...
class OutBlock(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, dim):
        ...
        if dim == '2d':
            ...
            conv = nn.ConvTranspose3d
        self.conv_out_1 = conv(in_channels, round(in_channels/2), kernel_size=3, stride=1, padding=1)
        self.conv_out_2 = conv(round(in_channels/2), n_classes, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU(inplace=True)
    def forward(self, x, dummy_tensor=None): 
        out = self.conv_out_1(x)
